    ...
    case "Object":
      ...
      for (k in o) {
        s +=
          t +
          k
            .replace(reDELIMITER, DELIMITER + DELIMITER)
            .replace(reStringDELIMITER, DELIMITER + StringDELIMITER) +
          StringDELIMITER +
          serialize(o[k]);
        t = DELIMITER;
      }
    ...
    case "RegExp":
      return (
        ...
        o.source
          ...
          .replace(reStringDELIMITER, DELIMITER + StringDELIMITER) +
        StringDELIMITER +
        (o.global * 4 + o.ignoreCase * 2 + o.multiline)
      );
    default:
      return "";
  }
  ...
  switch (s[0]) {
    ...
    case ArraySTART:
      ...
      while (s[0] != TERMINATOR) {
        ...
        if (s[0] != DELIMITER)
          throw "parse error: array entry doesn't end with delimiter or terminator";
        s = s.slice(1);
      }
      return [o, s.slice(1)];
    case DictionarySTART:
      ...
      while (s[1] != TERMINATOR) {
        ...
        if (s[0] == TERMINATOR) return [o, s.slice(1)];
        if (s[0] != DELIMITER)
          throw "parse error: dictionary entry doesn't end with delimiter or terminator";
      }
      return [o, s.slice(2)];
    case DateSTART:
      v = parse(s.slice(1));
